...
class RiskService:
    ...
    def evaluate(self, transaction: Transaction, training_job_id: str | None = None) -> tuple[RiskScore, Decision, str]:
        risk_score, decision, model_version = self.analyze(transaction, training_job_id=training_job_id)
        self._repository.save_analysis(transaction, risk_score, decision, model_version)

        return risk_score, decision, model_version

    def analyze(self, transaction: Transaction, training_job_id: str | None = None) -> tuple[RiskScore, Decision, str]:
    ...
    def _resolve_selected_model(self, training_job_id: str | None):
        if training_job_id is None:
            return None
        selected_model = self._model_loader.load_model_by_job_id(training_job_id)
        if selected_model is None:
            raise ValueError("Selected training job is unavailable. Use a succeeded baseline or sequence training job.")
    ...
    def _make_decision(score: float, decision_threshold: float | None = None) -> Decision:
        if score >= settings.risk_score_reject_min:
            ...
            if decision_threshold is not None:
                reason = "trained_model_high_risk_guardrail"
            return Decision(outcome=DecisionType.REJECT, reasons=[reason])

        if score <= settings.risk_score_approve_max:
            reason = "low_risk_profile"
            if decision_threshold is not None:
                reason = "trained_model_low_risk_guardrail"
            return Decision(outcome=DecisionType.APPROVE, reasons=[reason])
